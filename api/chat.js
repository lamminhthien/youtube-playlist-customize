// Vercel serverless function: proxies chat requests to Ollama.
// Supports:
...
// Env vars: OLLAMA_BASE_URL, OLLAMA_MODEL, OLLAMA_API_KEY (optional)
// Request body: { messages: [{role, content}], context: string, useExternal: bool, baseUrl?, model?, apiKey? }

const buildPrompt = (messages, context, useExternal) => {
  const userQuery = messages?.[messages.length - 1]?.content || "";
  const contextBlock = context ? `Cached video library (from user's playlists/channels):\n${context}\n` : "";
  const system = useExternal
    ? `You are a helpful assistant for a YouTube playlist app. You have access to the user's cached video library below. Answer the user's question helpfully. If the question is about videos, prioritize matching titles from the cached library and include video URLs when relevant. If the question is outside the library, answer from general knowledge but mention that it's outside their cached library. Keep answers concise and friendly.`
  ...
  return { system, userQuery, contextBlock };
};
...
const callOllama = async ({ baseUrl, model, apiKey, messages, context, useExternal }) => {
  const url = toOllamaChatUrl(baseUrl);
  const { system, userQuery, contextBlock } = buildPrompt(messages, context, useExternal);
...
export default async function handler(req, res) {
  ...
  const { messages, context, useExternal, baseUrl, model, apiKey } = req.body || {};
  ...
  // If external search not wanted, or no Ollama configured, return local fallback
  if (!wantExternal || !effectiveBaseUrl) {
    const answer = localFallbackAnswer(messages, context);
    return res.status(200).json({
      success: true,
      answer,
      source: !effectiveBaseUrl ? "local-no-ollama" : "local",
      model: null,
    });
  }
  ...
  try {
    ...
    const fallback = localFallbackAnswer(messages, context);
    return res.status(200).json({
      success: true,
...
// For unit tests
export const _testExports = { buildPrompt, callOllama, localFallbackAnswer, toOllamaChatUrl };
